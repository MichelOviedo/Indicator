...
/*
    private void builder(){
        ...
        RealmResults<Realm_viewTables> viewTables = realm.where(Realm_viewTables.class)
                ...
                .findAll();
        if (viewTables.get(0).getReading_score() <= 0) {
            viewLayoutList();
        }
        else{
            viewLatoutEdit();
        ...
        save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                for (int x = 0; x < listStudent.size(); x++) {
                    ...
                    results.get(0).setReading_score(ind);
                    //Como sincronizar la data o bien cada cuando se debe de sincronizar
                    showLog("Student Result: " + results.get(0).getReading_score());
                ...
                if (view.get(0).getReading_score() == -1)
                    goodJob(rs.getString(R.string.contentSave) + " '" + rs.getString(R.string.indLect) + "'");
                else
                    goodJob(rs.getString(R.string.contentEdit) + " '"+ rs.getString(R.string.indLect) + "'");


                view.get(0).setReading_score(1);
    ...
    private void viewLayoutList(){
        ...
        for (int x = 0; x < listStudent.size(); x++){
            ...
            showLog("Valor de indicator: " + stIndi.get(0).getReading_score());

            if (stIndi.get(0).getReading_score() == 0)
                extIndicator.setAdapter(adapter);
            if (stIndi.get(0).getReading_score() == 0.5)
                extIndicator.setAdapter(ada);
            if (stIndi.get(0).getReading_score() == 1)
