...
public class TabMath extends Fragment {
    ...
    private void builder(){
        ...
        type.setText(rs.getString(R.string.indicatorMath));
        count.setText(rs.getString(R.string.indicatorMathSb));
        ...
        titleEdit.setText(rs.getString(R.string.mateEdit));
        ...
        RealmResults<Realm_viewTables> viewTables = realm.where(Realm_viewTables.class)
                ...
                .findAll();
        if (viewTables.get(0).getMath_score() <= 0) {
            viewLayoutList();
        }
        else{
            viewLatoutEdit();
    ...
    public void agregarCabecera(){
        ...
        txtNombre.setText(rs.getString(R.string.indicatorM));
        ...
        save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                for (int x = 0; x < listStudent.size(); x++) {
                    ...
                    showLog("Student: " + results.get(0).getMath_score());
                    ...
                    results.get(0).setMath_score(ind);
                    //Como sincronizar la data o bien cada cuando se debe de sincronizar
                    showLog("Student Result: " + results.get(0).getMath_score());
                ...
                if (view.get(0).getMath_score() == -1)
                    goodJob(rs.getString(R.string.contentSave) + " '" + rs.getString(R.string.indMate) + "'");
                else
                    goodJob(rs.getString(R.string.contentEdit) + " '"+ rs.getString(R.string.indMate) + "'");

                view.get(0).setMath_score(1);
    ...
    private void viewLatoutEdit(){
        ...
        for (int x = 0; x < listStudent.size(); x++){
            ...
            if (stIndi.get(0).getMath_score() == 0)
                mala++;
            else
            if (stIndi.get(0).getMath_score() == 0.5)
                regular++;
            else
                buena++;
    ...
    private void viewLayoutList(){
        ...
        for (int x = 0; x < listStudent.size(); x++){
            ...
            showLog("Valor de indicator: " + stIndi.get(0).getMath_score());

            if (stIndi.get(0).getMath_score() == 0)
                extIndicator.setAdapter(adapter);
            if (stIndi.get(0).getMath_score() == 0.5)
                extIndicator.setAdapter(ada);
            if (stIndi.get(0).getMath_score() == 1)
